...
describe RocketPants::Routing do
  ...
  context 'a basic set of api routes' do
    ...
    it 'should recognise a path to a single version' do
      recognize_path('/1/a').should == {controller: 'test', action: 'echo', version: '1', format: 'json', prefix: nil, required_prefix: true }
      recognize_path('/2/b').should == {controller: 'test', action: 'echo', version: '2', format: 'json', prefix: nil, required_prefix: true }
      recognize_path('/3/b').should == {controller: 'test', action: 'echo', version: '3', format: 'json', prefix: nil, required_prefix: true }
    end
  ...
  context 'api routes with prefix support' do
    ...
    before :each do
      draw_routes do
        api version: 1, prefix: 'v', required_prefix: false do
          get 'a', to: 'test#echo'
        end
        api versions: %w(2 3) do
          get 'b', to: 'test#echo'
        end
        api versions: %w(4 5), prefix: 'v' do
          get 'c', to: 'test#echo'
        end
      end
    end
    ...
    it 'should recognise a path with and without prefix when allow_prefix is given' do
      recognize_path('/v1/a').should == {controller: 'test', action: 'echo', version: 'v1', format: 'json', prefix: 'v', required_prefix: false }
      recognize_path('/1/a').should == {controller: 'test', action: 'echo', version: '1', format: 'json', prefix: 'v', required_prefix: false }
    end
    ...
    it 'should recognise a path only with version when require_prefix is given' do
      recognize_path('/v4/c').should == {controller: 'test', action: 'echo', version: 'v4', format: 'json', prefix: 'v', required_prefix: true }
      recognize_path('/v5/c').should == {controller: 'test', action: 'echo', version: 'v5', format: 'json', prefix: 'v', required_prefix: true }
  ...
  it 'should not let you draw a route with an invalid version' do
    expect_options(version: '  ').to raise_error(ArgumentError)
    expect_options(version: '1.1.1.1').to raise_error(ArgumentError)
